...
Teacher.editgrade = (req, result) => {
      connection.query(`SELECT stu.Name as 'StudentName',15phut_1,15phut_2,15phut_3,15phut_4,45phut_1,45phut_2,giuaki,cuoiki
                ...
                WHERE  s.Name = "${req.body.subjectName}" and a.Teacher_ID="${req.body.TeacherID}" and a.Class=g.Class`, (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(err, null);
        return;
...
Teacher.viewattendance = (req, result) => {
    ...
    WHERE s.Name = "${req.body.subjectName}" and ass.Teacher_ID="${req.body.id}" and ass.Class=a.Class and a.Class="${req.body.class}" `, (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(err, null);
        return;
