...
Parent.timetable = (paID, result) => {
    connection.query(`SELECT s.Class,Time,Day_Week,a.Teacher_ID,t.Name as 'TeacherName'
                FROM Assignment a join Timetable tb on tb.Assignment_ID=a.id join Teacher t on t.id=a.Teacher_ID,Student s,Subject sub,Parent p
                WHERE sub.id=a.Subject_ID and p.id = "${paID}" and s.Class=a.Class and p.id_student=s.id`, (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(err, null);
        return;
